 ...
 *     being 1, 1, 2, 2 and 3 respectively. It does not matter what you leave beyond the 
 *     returned k (hence they are underscores).
 * Example 2:
 *     Input: nums = [0,0,1,1,1,1,2,3,3]
 *     Output: 7, nums = [0,0,1,1,2,3,3,_,_]
 *     Explanation: Your function should return k = 7, with the first seven elements of nums 
 *     being 0, 0, 1, 1, 2, 3 and 3 respectively. It does not matter what you leave beyond 
 *     the returned k (hence they are underscores).
 * 
 * Solution Discussion and generalization with atmost k duplicates:
...
public:
    int removeDuplicates(vector<int>& nums) {
        ...
        int cnt = 0; // count of same numbers
        
        for(int i=0; i<n; i++){
            
            // increase  the count if same number appears
            ...
            // keep putting numbers until the count is less than 2
            if(cnt>0 && cnt <=2 && j<n){
                nums[j] = nums[i];
                j++;
            }
